...
type App struct {
	...
	fanRamp       *FanRamp
	spraySched    *clock.SprayScheduler
	antifreeze    *clock.AntifreezeWindow
	fillGuard     *interlock.FillGuard
	segmentLeases *interlock.SegmentLeaseRegistry
	cycleMu       sync.Mutex
	cycleCancels  map[model.TowerID]context.CancelFunc
}
...
func New(cfg config.Config) (*App, error) {
	...
	a.wireDriftSensors()
	a.driftWin = drift.NewWindow(a.clk, start, time.Duration(cfg.DriftHoldMinutes)*time.Minute, cfg.DriftMaxPPM)
	a.fanRamp = NewFanRamp(a.clk, cfg.FanRampSteps, cfg.ProcessTick())
	a.spraySched = clock.NewSprayScheduler(a.clk)
	a.antifreeze = clock.NewAntifreezeWindow(a.clk, time.Duration(cfg.AntifreezeWindowMin)*time.Minute)
	a.fillGuard = interlock.NewFillGuard(12.0)
	a.segmentLeases = interlock.NewSegmentLeaseRegistry(30 * time.Second)
	a.cycleCancels = make(map[model.TowerID]context.CancelFunc)
	a.alarms.Manager().Register("FILL_BLOCK", "Fill media pressure block active")
